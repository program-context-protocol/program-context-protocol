...
def test_init_generated_controls_yaml_parses_and_has_thirtysix_controls(tmp_path):
    import yaml
    runner = CliRunner()
    runner.invoke(cli, ["init", "--path", str(tmp_path)])
    data = yaml.safe_load((tmp_path / ".pcp" / "controls.yaml").read_text())
    assert len(data["controls"]) == 36
    assert {c["id"] for c in data["controls"]} == {f"CTRL-{i:03d}" for i in range(1, 37)}
